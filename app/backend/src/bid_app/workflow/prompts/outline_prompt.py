...
3. 输出**层级化**结构:一级 = 章(分组,不直接生成正文),二级 = 节(叶子,
   每节会被 LLM-2 单独写正文)。若内容确实需要再细分,可允许三级,但
   只有叶子节点参与正文生成。
4. 一级章节数控制在 5-8 个;每个一级下挂 2-5 个二级节;总叶子数控制在
   12-25 个之间。
5. 每个**叶子节**必须含明确的关键点列表,便于后续撰写时聚焦;一级章节
   只给标题,不需要 key_points / target_pages。
...
要求:
- 一级 5-8 个,二级总数 12-25 个为宜
- target_pages 根据打分权重和内容深度分配 1-6 页 (**只在叶子上设**)
- key_points 每个叶子 3-7 个
- matched_scoring_items 列出本节主要覆盖的打分项
- 一级章节不要 ``key_points`` / ``target_pages`` / ``summary``,只给 ``title`` + ``children``
