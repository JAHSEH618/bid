...
def _final_path(project_id: int) -> Path:
    return _project_dir(project_id) / _BLACKBOARD_FILENAME
...
async def write_blackboard(project_id: int, html: str) -> Path:
    ...
    try:
        ...
        async with session_factory() as s:
            await s.execute(
                sa.text(
                    "UPDATE projects SET blackboard_path=:p WHERE id=:i"
                ),
...
async def read_blackboard(project_id: int) -> str:
    ...
    path = _final_path(project_id)
    if not path.exists():
        raise BlackboardMissing(
            f"blackboard.html missing on disk for project {project_id}"
        )
...
async def delete_blackboard(project_id: int) -> None:
    ...
    try:
        ...
        async with session_factory() as s:
            await s.execute(
                sa.text(
                    "UPDATE projects SET blackboard_path=NULL WHERE id=:i"
                ),
                {"i": project_id},
            )
            await s.commit()
        ...
        log.exception(
            "blackboard_db_clear_failed", project_id=project_id
        )
