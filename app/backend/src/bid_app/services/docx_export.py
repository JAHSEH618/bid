...
PR-M6-2:支持 chapter scope 单章导出。共享 ``_run_pandoc_pipeline`` 与
全局 Redis 锁;单章 tmp 文件名 ``chapter_{chapter_id}.{job_id}.tmp.docx``,
final 路径由调用方决定。
"""
...
async def export_docx(
    *,
    ...
    async with _serial_lock(redis_url):
        return await _run_pandoc_pipeline(
            markdown=markdown,
            work_dir=project_dir / "docx-build",
            out_path=out_path,
...
async def export_chapter_docx(
    *,
    ...
    与 ``export_docx`` 共享全局 Redis 锁与 pandoc pipeline,只是输出路径与
    work 目录按 chapter_id 隔离,避免与整本导出 / 其它章节并发跑时的
    mermaid 中间文件互相覆盖。

    返回 tmp 路径;调用方在 DB done 之后 atomic rename 成 final 路径
    (建议 ``chapter_{chapter_id}.docx``)。
    """
    suffix = f".{job_id}" if job_id is not None else ""
    work_dir = project_dir / f"docx-build-chapter-{chapter_id}"
    out_path = project_dir / f"chapter_{chapter_id}{suffix}.tmp.docx"
...
@asynccontextmanager
async def _serial_lock(redis_url: str) -> AsyncIterator[None]:
    """同进程锁 + Redis 锁双层串行 (D-H + D-CV)。

    单章和整本共用同一对锁,保证 Mermaid CLI / Chromium / LibreOffice 都
    不会被并发触发。两道锁分开管理生命周期:同进程锁先拿,Redis 锁后拿
    (挂在 ``async with`` 退出时自动释放)。
    """
    try:
...
async def _run_pandoc_pipeline(
    *,
    markdown: str,
    work_dir: Path,
    out_path: Path,
    reference_doc: Path | None,
    on_stage: OnStageCallable | None,
) -> Path:
    """共享 mermaid 预渲染 + pandoc 直转;chapter / project 都走这里。"""
    work_dir.mkdir(parents=True, exist_ok=True)

    # 1. mermaid 预渲染(图片用相对路径,后面 pandoc 用 --resource-path 解析)
    inlined = await _render_mermaid(markdown, work_dir)
    ...
    args = [
        ...
        str(out_path),
        "--resource-path",
        str(work_dir),  # 让相对图片路径(./mmd_0.png)能解析
        "--standalone",
    ]
...
async def _export_docx_inner(
    ...
    *,
    ...
    """旧入口,保留给 ``cli/run_local`` 等少量 caller 用;实际走 pipeline 共享路径。"""
    suffix = f".{job_id}" if job_id is not None else ""
    out_path = project_dir / f"proposal{suffix}.tmp.docx"
    return await _run_pandoc_pipeline(
        markdown=markdown,
